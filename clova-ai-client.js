...
[중요: 위 3줄 이후 어떤 내용도 추가하지 말고 즉시 종료할 것]`;
  ...
  generateMockAffirmations(problemIds, toneId) {
    ...
    const toneStyles = {
      ...
      3: { // 결의에 찬 강한
        suffix: ["것이다!", "할 것이다!", "다!", "반드시 할 것이다!"],
        prefix: ["나는", "나는 반드시", "어떤 어려움도"]
      }
    };
    ...
    const mockAffirmations = {
      ...
      4: { // 과거를 후회해요
        ...
        3: ["과거는 나를 정의할 수 없다!", "나는 후회를 완전히 떨쳐내고 미래를 개척할 것이다!", "어떤 과거도 나의 앞길을 막을 수 없다!"]
      },
      5: { // 스스로가 부족하게 느껴지고 자신감이 없어요
    ...
    if (allAffirmations.length < 3) {
      const defaultByTone = {
        ...
        3: ["나는 강력한 의지로 성장을 이뤄낼 것이다!", "어떤 것도 나의 발전을 막을 수 없다!", "나는 확고한 신념으로 최고가 될 것이다!"]
      };
